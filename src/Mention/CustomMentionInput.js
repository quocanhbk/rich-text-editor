...
const CustomMentionInput = (props) => {
  ...
  const onSearchChange = useCallback(
    ({ value }) => setSuggestions(defaultSuggestionsFilter(value, props.data)),
    [props.data]
  );
  ...
  useEffect(() => {
    ...
    let dateData = Object.values(models)
      .filter((_) => _.type === "//mention")
      .map((_) => _.data.mention.name);
    let mentionData = Object.values(models)
      .filter((_) => _.type === "mention")
      .map((_) => _.data.mention.name);
